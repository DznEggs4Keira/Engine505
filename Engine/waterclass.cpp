...
bool WaterClass::Initialize(ID3D11Device* device, WCHAR* textureFilename, int waterWidth, int waterHeight)
{
	int index;
	float height = 0.7f;
	bool result;
	...
	m_waterWidth = waterWidth;
	m_waterHeight = waterHeight;

	// Create the structure to hold the terrain data.
	m_heightMap = new HeightMapType[m_waterWidth * m_waterHeight];
	if (!m_heightMap)
	{
		return false;
	}
	...
	for (int j = 0; j < m_waterHeight; j++)
	{
		for (int i = 0; i < m_waterWidth; i++)
		{
...
bool WaterClass::CalculateNormals()
{
	...
	normals = new VectorType[(m_waterHeight - 1) * (m_waterWidth - 1)];
	if (!normals)
	{
		return false;
	}
	...
	for (j = 0; j < (m_waterHeight - 1); j++)
	{
		for (i = 0; i < (m_waterWidth - 1); i++)
		{
			index1 = (j * m_waterHeight) + i;
			index2 = (j * m_waterHeight) + (i + 1);
			index3 = ((j + 1) * m_waterHeight) + i;
	...
	for (j = 0; j < m_waterHeight; j++)
	{
		for (i = 0; i < m_waterWidth; i++)
		{
			...
			// Bottom right face.
			if ((i < (m_waterWidth - 1)) && ((j - 1) >= 0))
			{
				index = ((j - 1) * (m_waterHeight - 1)) + i;
			...
			// Upper left face.
			if (((i - 1) >= 0) && (j < (m_waterHeight - 1)))
			{
				index = (j * (m_waterHeight - 1)) + (i - 1);
			...
			// Upper right face.
			if ((i < (m_waterWidth - 1)) && (j < (m_waterHeight - 1)))
			{
				index = (j * (m_waterHeight - 1)) + i;
...
bool WaterClass::InitializeBuffers(ID3D11Device* device)
{
	VertexType* vertices;
	...
	m_vertexCount = (m_waterWidth - 1) * (m_waterHeight - 1) * 6;
	...
	for (j = 0; j < (m_waterHeight - 1); j++)
	{
		for (i = 0; i < (m_waterWidth - 1); i++)
		{
			index1 = (m_waterHeight * j) + i;          // Bottom left.
			index2 = (m_waterHeight * j) + (i + 1);      // Bottom right.
			index3 = (m_waterHeight * (j + 1)) + i;      // Upper left.
			index4 = (m_waterHeight * (j + 1)) + (i + 1);  // Upper right.
...
void WaterClass::CalculateTextureCoordinates()
{
	...
	incrementValue = (float)REPEAT / (float)m_waterWidth;

	// Calculate how many times to repeat the texture.
	incrementCount = m_waterWidth / REPEAT;
	...
	for (j = 0; j < m_waterHeight; j++)
	{
		for (i = 0; i < m_waterWidth; i++)
		{
			...
			m_heightMap[(m_waterHeight * j) + i].tu = tuCoordinate;
			m_heightMap[(m_waterHeight * j) + i].tv = tvCoordinate;
